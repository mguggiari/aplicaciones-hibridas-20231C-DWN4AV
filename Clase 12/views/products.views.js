...
function createProductListPage(products) {
    ...
    for (let i = 0; i < products.length; i++) {
        html += `<li>${products[i].name} <a href="/products/${products[i].id}">Ver</a> <a href="/products/${products[i].id}/edit">Modificar</a> <a href="/products/${products[i].id}/delete">Eliminar</a></li>`
    }
...
function createDeleteProductFormPage(product) {
    ...
    html += `<form action="/products/${product.id}/delete" method="POST">
